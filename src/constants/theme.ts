// src/lib/theme.ts
// ────────────────────────────────────────────────
// Central place for all color schemes, shadows, fonts, gradients, etc.
// ────────────────────────────────────────────────

export const THEME = {
  // Parchment / Fantasy base (used in Binder & Rules)
  bg: 'bg-[#e8e4d9]',                    // light parchment
  panel: 'bg-[#fdfbf7] border-2 border-[#d4c5a9]',
  header: 'bg-[#2c241b] text-[#eebb4d]', // dark leather + gold
  accent: 'text-[#8b4513]',              // saddle brown
  goldBorder: 'border-[#daa520]',
  silverBorder: 'border-[#a9a9a9]',

  // Buttons
  buttonPrimary:
    'bg-gradient-to-b from-[#5c4033] via-[#2c241b] to-[#1a1008] text-[#f5deb3] border-4 border-[#1a1008]',
  buttonAction:
    'bg-gradient-to-b from-[#4ade80] via-[#2e8b57] to-[#14532d] text-white border-4 border-[#14532d]',
  buttonDanger:
    'bg-gradient-to-b from-[#ef4444] via-[#8b0000] to-[#450a0a] text-white border-4 border-[#450a0a]',
  buttonGold:
    'bg-gradient-to-b from-[#faeebf] via-[#daa520] to-[#b8860b] text-[#2c241b] border-4 border-[#8b4513]',

  // Inputs / text fields
  input: 'bg-[#fffef8] border border-[#d4c5a9] font-serif text-[#2c241b]',
};

// Shared metallic / fantasy styling helpers
export const METALLIC_SHADOW =
  'shadow-[0_4px_4px_rgba(0,0,0,0.3),inset_0_1px_0_rgba(255,255,255,0.5)]';

export const METALLIC_FONT = 'font-serif font-bold tracking-wide';

// ────────────────────────────────────────────────
// View-specific themes
// ────────────────────────────────────────────────

export const HUB_THEME = {
  bg: 'bg-gradient-to-b from-blue-800 to-slate-900',
  panel: 'bg-blue-900/80 border-2 border-blue-400/40 shadow-lg backdrop-blur-sm',
  textMain: 'text-blue-50 font-sans drop-shadow-md',
  textAccent: 'text-cyan-300',
  barBg: 'bg-blue-950 border border-blue-500',
  barFill: 'bg-gradient-to-r from-blue-500 via-cyan-400 to-blue-300',
};

export const QUEST_THEME = {
  bg: 'bg-gradient-to-b from-emerald-900 via-[#5d534a] to-[#3e3730]',
  tray: 'bg-emerald-900/80 border-2 border-emerald-700/50 shadow-2xl backdrop-blur-sm',
};

export const BINDER_THEME = {
  bg: 'bg-gradient-to-b from-pink-900 to-purple-950',
  page: 'bg-[#fdf4f8]',
  spine: 'bg-[#4c1d95]',
  ring: 'bg-gradient-to-b from-stone-300 via-white to-stone-400',
  accent: 'text-[#e9d5ff]',
};